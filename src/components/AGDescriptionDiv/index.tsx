const AGDescriptionDiv = () => {
    return (
        <>
      
            <div className="bg-[#060A0A]/65  w-[300px] sm:w-[400px] md:w-[550px] lg:w-[700px] p-5 rounded-[45px] text-white 
                absolute top-[-90px] lg:top-[-90px]">
                <p className="mt-4 mx-4 sm:mx-16 sm:mt-12 md:mt-6 text-justify charter">In a world ravaged by pollution and climate collapse, 
                    the wealthy have escaped to airborne cities, leaving the poor to survive in the scorched ruins below. 
                    You are a scavenger — a member of the Sandwalkers Guild — risking your life to collect vital resources 
                    from the remnants of the old world.After a devastating crash, you're stranded in the wasteland. 
                    To return home, you'll need to repair your ship, survive the chaos, and prove your worth in a 
                    world where only the toughest endure.
                </p>
                <p className="mt-0 mx-4 sm:mx-16 text-justify charter">
                    Will you rise again — or be consumed by the ruins of the past?
                </p>
            </div>
        
        </>
    )
}

export default AGDescriptionDiv